 ...
 * 
 * 1. 경로 구분자 (Seperator)
 *    1) Windows: \ (역슬래시) (자바에서는 \를 이스케이프 처리: \\)
 *    2) Mac/Linux : / (슬래시)  
 ...
 *       (2) Windows에서 개발할때도 슬래시를 사용한다. (대부분 슬래시, 역슬래시 모두 경로 구분자로 인식하도록 호환성을 제공)
 * 
 * 2. 예약된 디렉터리
 *    1) 현재 디렉터리: .
 *    2) 상위 디렉터리: .. 
...
public class Main {
  ...
  public static void handling() {
    ...
    //------ 1. 생성할 디렉터리를 File 객체로 생성 (디렉터리를 여기서 만드는건 아닙니다.)
    File dir = new File("C:/study/Java/storage");
    
    //------ 2. 디렉터리 생성(없으면 만드는 방식으로 진행합니다.) 
    if(dir.exists() == false) {
      dir.mkdir();
      System.out.println("디렉터리가 생성돼었습니다.");
    }
    else {
    ...
    //---- 실습. 현재 날짜를 디렉터리 이름으로 사용해서 생성하시오. (예: 2025/07/10)
  ...
  public static void information() {
    ...
    for (File file : files) {
      ...
      // 이름
      sb.append(String.format("%10s", file.getName())).append("\n");
    }
  ...
  //            D:/2025/07/10 삭제 -> D:/2025/07 삭제 -> D:/2025 삭제
    ...
    deleteDirectoryTree(dir.getParent()); //-- 상위 디렉토리 정보를 넘깁니다.
  }
