...
public class Main {
  ...
  public static void decimalFormat() {
     ...
     * 2. 천 단위 구분 기호(,) 또는 소수 자릿수 지정, 0 채우기 등의 다양한 숫자 형식 지정에 사용됩니다.
     * 3. 주요 패턴 기호
     *    1) 0  빈자리를 0으로 채움
     *    2) #  빈자리는 표시하지 않는다.
     *    3) ,  천 단위 구분 기호
    ...
    System.out.println(df.format(1234.56)); //--- 자동 반올림
    
    // 패턴 적용 후 확인 - 2
    df.applyPattern("0.00"); //--------------------- 소수 이하 2자리로 표기
    System.out.println(df.format(1234.567)); //--- 자동 반올림
  ...
  public static void optional() {
     ...
     * 3. 저장한 값을 꺼내서 사용할 때 null에 대한 대처가 가능해서 NPE(NullPointerException) 발생 가능성을 줄입니다.
    ...
    Optional<String> opt2 = Optional.ofNullable(str2); // null이 될 수 있는 값을 감싸는 경우 사용합니다.
