...
SELECT * FROM tbl_employee WHERE salary BETWEEN 3000000 AND 5000000;
...
SELECT position FROM tbl_employee GROUP BY position HAVING AVG(salary) >= 5000000;

# 8. 직급이 "과장"인 직원 수 조회하기
SELECT COUNT(*) FROM tbl_employee WHERE position = "과장";

# 9. 입사일 과거 순으로 조회하기
SELECT * FROM tbl_employee ORDER BY hire_date ASC;

# 10. 직급 순으로 조회하되, 같은 직급은 최신 고용 순으로 조회하기
SELECT * FROM tbl_employee ORDER BY position ASC, hire_date DESC;

# 11. 가장 급여가 높은 직원 1명 조회하기
SELECT * FROM tbl_employee ORDER BY salary DESC LIMIT 0, 1;

# 12. 가장 먼저 입사한 직원 1명 조회하기
SELECT * FROM tbl_employee ORDER BY hire_date ASC LIMIT 0, 1;