...
/*
    ...
    100개의 행이 조회됩니다.
*/
SELECT player_name AS 선수이름, CASE
                       WHEN position = "DF" THEN "수비수"
                       WHEN position = "MF" THEN "미들필더"
...
/*
    ...
    480개의 행이 조회됩니다.
*/
SELECT player_name AS 선수이름, IFNULL(nation, "대한민국") AS 국적 
FROM tbl_player
...
FROM tbl_player p
JOIN tbl_team t ON p.team_id = t.team_id;
...
FROM tbl_player p
JOIN tbl_team t ON p.team_id = t.team_id
JOIN tbl_stadium s ON s.hometeam_id = t.team_id;

# USING 활용
SELECT player_name, position, nation, team_name, stadium_name
FROM tbl_player 
JOIN tbl_team  USING(team_id)
JOIN tbl_stadium  USING(stadium_id)
