...
# 부서id, 부서명, 직원아이디, 직원명 Cross Join으로 조회하기
...
/* 
  ...
  2. 테이블 크기, 인덱스 유무, 조건절의 선택도(조건 필터리 결과가 적을 것으로 예상되는 테이블을 Drive Table로 선정)
  등을 이용해서 Drive 테이블을 선택합니다.
  3. 과거 DB는 FROM절에 작성한 테이블을 Drive Table로 인식하기도 했습니다. 
*/

# 모든 직원들 부서id, 부서명, 직원id, 직원명 조회하기
SELECT d.dept_id, dept_name, emp_id, emp_name
FROM tbl_department d
...
EXPLAIN SELECT d.dept_id, dept_name, emp_id, emp_name
FROM tbl_department d
INNER JOIN tbl_employee e
...
/*
  ...
  3. 어느 쪽 테이블의 모든 데이터를 항상 포함할지에 따라 다음 세 가지로 나뉩니다.
    1) LEFT OUTER JOIN: JOIN 절 왼쪽에 있는 테이블의 모든 데이터가 항상 조회됩니다.
    2) RIGHT OUTER JOIN: JOIN 절 오른쪽에 있는 테이블의 모든 데이터가 항상 조회됩니다.
    3) FULL OUTER JOIN: JOIN 절 양쪽에 있는 테이블의 모든 데이터가 항상 조회됩니다.
*/
...
SELECT d.dept_location, COUNT(e.emp_id) -- 직원 수를 구하기 위해서 모든 직원이 값을 가지고 있는 emp_id(pk)를 count함수에 전달.
FROM tbl_employee e 
