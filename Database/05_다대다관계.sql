...
# M             N (일대다 관계가 모두 성립하는 경우 다대다 관계입니다. 다대다 관계는 두 테이블을 직접 관계지을 수 없습니다.)

# 다대다 관계는 일대다 관계 2개로 구성합니다.
# 일대다 관계 2개를 구성하려면 테이블이 3개 필요합니다.
...
CREATE TABLE IF NOT EXISTS tbl_enrollment(
  ...
  UNIQUE KEY(stud_id, subj_id) # 학생 아이디와 과목 아이디는 중복 불가( 한 학생이 한 과목을 중복 수강하지 못하도록)
) ENGINE=InnoDB;
# 수강현황 테이블 삭제
