...
CREATE TABLE IF NOT EXISTS tbl_product(
  ...
  FOREIGN KEY(category_id) REFERENCES tbl_category(category_id) # 외래키 설정: 현재 테이블의 category_id는 category 테이블의 category_id 칼럼이 가진 값만 가질 수 있습니다.(참조 무결성) 
    ON UPDATE CASCADE # 부모 테이블의 PK가 수정되면 자식 테이블의 FK가 함께 수정됩니다.
    ON DELETE CASCADE # 부모 테이블의 PK가 삭제되면 자식 테이블의 FK가 함께 삭제됩니다.
) ENGINE=InnoDB;

# 연습) 제품의 이미지를 등록하고자 합니다. 
# 이미지 아이디, 제품 아이디, 이미지 url 주소, 메인/서브 이미지 구분, 최초 등록 일시 정보를 저장하도록 tbl_product_image 이미지 테이블을 만드세요.

CREATE TABLE IF NOT EXISTS tbl_product_image(
  image_id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  product_id INT NULL,
  image_url VARCHAR(100) NOT NULL,
  image_isMain BOOLEAN DEFAULT FALSE,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
