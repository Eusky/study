...
FROM tbl_employee
WHERE position = (SELECT position FROM tbl_employee WHERE emp_id = 1001); -- 단일 행 서브쿼리 (연산자: =, !=, >, >=, <, <=)
...
# 직원 아이디가 1003인 직원 정보와 전체 직원의 급여 평균을 함께 조회하세요.
SELECT emp_id, dept_id, emp_name, position, gender, hire_date, salary,
        (SELECT AVG(salary) FROM tbl_employee) AS 전체평균
FROM tbl_employee
WHERE emp_id = 1003;

/*
  상관 서브쿼리 (Correlated Sunquery)
  1. 서브쿼리가 메인쿼리의 값을 참조하는 방식의 쿼리입니다.
  2. 서브쿼리가 메인쿼리의 각 행마다 한 번씩 실행되고, 메인 쿼리의 갑을 서브쿼리에서 사용합니다.
*/
