...
# 결과
# type = const : PK 또는 UK를 WHERE 절에서 상수 값과 비교하여 항상(const) 하나의 행만 반환하는 접근 방식
# rows = 1     : MySQL이 검사할 것으로 예상되는 행의 갯수 
...
# 결과
# type = ALL : 테이블의 모든 행을 처음부터 모두 읽는 Full Table Scan 방식
...
ON tbl_employee(emp_name ASC); -- 직원 테이블의 직원명에 인덱스를 설정합니다.
