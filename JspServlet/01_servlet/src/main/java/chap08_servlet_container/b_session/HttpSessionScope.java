...
public class HttpSessionScope extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 2. 웹 애플리케이션의 프로토콜인 HTTP는 기본적으로 어떤 상태도 기억하지 않는 stateless 이므로 각 사용자의 정보(로그인 상태, 장바구니 등)를 
		 *    서버가 일정 시간동안 기억하도록 도와줍니다. 
		 ...
		 * 5. 세션 데이터는 서버 메모리에 저장되며, 각 사용자마다 고유의 식변자인 JSESSIONID 를 할당합니다. 
		 *    이 값은 기본적으로 쿠키에 JSSESIONID라는 이름으로 저장됩니다. 
		 * 6. HttpSession 객체 얻기
		 *    HttpSessopn session = request.getSession();
		 * 7. 데이터 처리 예시
