...
public class FirstRedirect extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	   ...
	   * redirect 
	   * 
	   * 1. 서버가 클라이언트에게 "새로운 주소로 다시 요청하세요" 라고 지시하는 방식입니다. (응답)
	   * 2. redirect 지시를 받은 클라이언트는 다시 새로운 주소로 이동하므로 URL 변경을 확인할 수 있습니다. 
	   * 3. 최초 요청시 사용한 request와 response가 전달되지 않고 별개의 새로운 요청을 전달합니다. 
	   * 4. redirect 시 다른 서버로 이동할 수 있습니다.  
	   * 5. DB의 데이터 변경 작업(INSERT, UPDATE, DELETE) 이후 redirect로 이동으로 새로고침으로 인한 중복 제출(submit) 등을 방지할 수 있습니다.
