...
public class SaveCookie extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	   ...
	   * 4. 주요 메소드 
	   *    1) setMaxAge(int): 쿠키 유효시간(초) 설정. 0은 쿠키 삭제를 의미, 음수(-1)는 브라우저 종료 시 쿠키 삭제를 의미.
	   *    2) setPath(String): 쿠키가 유효경로 설정. 해당 경로 이하에서만 쿠키가 유효.
	   *    3) setSecure(boolean): true 시 HTTPS 통신에서만 쿠키 전송을 허용.
	   *    4) setHttpOnly(boolean): true 시 JavaScript로 접근하지 못하도록 쿠키 보호.
	   */
	  ...
	  emailCookie.setMaxAge(-1); // 브라우저 종료시 삭제되는 쿠키(session cookie). setMaxAge 생략시에도 session cookie가 됩니다. 
