...
1. <c:out>     값 출력
2. <c:set>     서블릿 컨테이너의 데이터 저장 (setAttribute)
...
<%
  request.setAttribute("names", new String[] {"김경진", "권형택", "이유천", "박진성", "조기완"});
%>
<c:forEach var="name" items="${names}" varStatus="vs">
  <div>이름: ${name}, 인덱스: ${vs.index}, 몇 번째 요소인가? ${vs.count }</div>
</c:forEach> 

<%
  Product product1 = new Product();
  product1.setModel("리모컨");
