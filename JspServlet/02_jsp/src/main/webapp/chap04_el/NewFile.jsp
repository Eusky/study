...
<%-- 
  ...
  (<%= %> 또는 out.print() )
  2. HTML과 JSP 코드의 혼재로 인한 코드 가독성 문제를 해결합니다. 
  3. Servlet 컨테이너(request, session, application, pageContext)에 저장된 값을 쉽게 접근할 수 있도록 설계되었습니다.
  4. 기본 문법
     ${값} 
  5. 일반 자바 변수는 EL 문법을 사용할 수 없고 서블릿 컨테이너에 저장된 값만 EL 문법을 사용할 수 있습니다. 
...
<div>a > b : ${a > b}, a > b : ${a gt b}</div>
<div>a >= b : ${a >= b} a >= b : ${a ge b}</div>
<div>a < b : ${a < b}, a < b : ${a lt b}</div>
<div>a <= b : ${a <= b}, a <= b : ${a le b}</div>
<div>a == b : ${a == b}, a == b : ${a eq b}</div>
<div>a != b : ${a != b}, a != b : ${a ne b}</div>
 ...
 <div>a와 b 문자열 연결(+는 지원이 안되고 +=는 지원이 됨) : ${a += b}</div>
 ...
 <div>List가 비어있지 않은가? ${not empty list}</div>
 <hr>
 
 <%-- request이 데이터 처리 방법 확인을 위한 링크 --%>
