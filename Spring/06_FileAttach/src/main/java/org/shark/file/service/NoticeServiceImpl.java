...
@Transactional //---- 서비스 클래스 레벨에 설정한 @Transactional에 의해서 모든 메소드는 트랜잭션 처리 됩니다. 
@Service
...
public class NoticeServiceImpl implements NoticeService {
  ...
  @Transactional(readOnly = true) //---- 읽기 전용 최적화를 통해 트랜잭션 매니저의 불필요한 동작을 방지하여 성능을 향상할 수 있다. 
  @Override
  ...
  public boolean addNotice(NoticeDTO notice, List<MultipartFile> files) {
    ...
    try {
      System.out.println("공지사항 등록 이전 nid: " + notice.getNid()); //------ nid = 0
      //------ 공지사항 DB에 등록하기 
      int addedNoticeCount = noticeDAO.insertNotice(notice); //-------- INSERT 수행하면서 파라미터 notice의 nid 필드에 자동 생성된 PK값이 저장됩니다. 
      System.out.println("공지사항 등록 이후 nid: " + notice.getNid()); //------ nid = auto_increment된 PK값
