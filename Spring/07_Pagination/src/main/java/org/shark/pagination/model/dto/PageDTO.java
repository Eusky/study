...
public class PageDTO {
  ...
  private int size = 20; //------ 한 페이지에 표시할 항목의 갯수 (요청 파라미터, 디폴트 20)
  private int offset; //--------- 각 페이지의 시작 인덱스 (PageUtil에서 계산)
  
  private int itemCount; //------ 전체 항목의 갯수 (DB에서 COUNT() 함수로 결과 계산)
  private int pageCount; //------ 전체 페이지의 갯수 (PageUtil에서 계산)
  private int beginPage; //------ 현재 블록의 시작페이지 (PageUtil에서 계산)
