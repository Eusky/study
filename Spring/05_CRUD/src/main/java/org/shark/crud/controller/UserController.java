...
public class UserController {
  ...
  @GetMapping("/login")
  public String loginForm(String url, HttpServletRequest request, Model model) {
  ...
  public String login(RedirectAttributes redirectAttr, // 로그인 실패 시 에러메시지 담아서 다시 로그인 페이지로 redirect 하기 위함입니다. 
                      HttpServletRequest request,      // 로그인 성공 시 세션에 nickname 올리기 위함 and 컨텍스트 패스 사용하기 위함입니다.
    ...
    if(loginUser == null) {
      redirectAttr.addFlashAttribute("error", "아이디와 비밀번호를 확인하세요.").addAttribute("url", url);
      ...
      HttpSession session = request.getSession(true);  //-------- true : 디폴트. 기존 세션이 존재하면 가져오고, 없으면 새로 만들어서 가져옵니다.
  ...
  @PostMapping("/signup")
  public String singup(UserDTO user, RedirectAttributes redirectAttr) {
    boolean result = userService.signUp(user);
    if(!result) {
      redirectAttr.addFlashAttribute("error", "회원 가입이 실패했습니다. 다시 시도하세요.");
      return "redirect:/user/signup";
    }
    redirectAttr.addFlashAttribute("msg", "회원 가입이 성공했습니다.");
    return "redirect:/";
  }
