...
public class MvcControllerC {
   ...
   * 3. Map과 유사한 Key-Value 형식으로 데이터를 저장합니다. 
   * 4. 주요 메소드
   *    1) addAttribute(String key, Object value)
   *    2) asMap()
   * 5. 리다이렉트 시에는 Model에 저장된 데이터는 소멸됩니다. 
   */
  ...
  public String submit(@ModelAttribute() BoardDTO board) { // Model에 board라는 이름으로 BoardDTO board 객체가 저장됩니다. 
           // name = "board"를 생략하면 (@ModelAttribute BoardDTO board) Model에 boardDTO라는 이름으로 BoardDTO board 객체가 저장됩니다. 
           // JSP에서 ${boardDTO}로 확인 가능.
           // @ModelAttribute를 생략할 수 있습니다. (스프링 3.2 이후) 이 경우 Model에 boardDTO라는 이름으로 BoardDTO 객체가 저장됩니다. 
   ...
   * 
   * 1. 리다이렉트 시 데이터를 일시적으로 저장하는데 사용하는 인터페이스입니다. 
   * 2. 리다이렉트 시 Model에 저장된 데이터는 소멸되고(새로운 요청이기 때문에) RedirectAttribute를 사용해야 합니다. 
   * 3. 주요 메소드
   *    1) addAttribute(String key, Object value) 
   *       (1) 전달할 데이터를 쿼리 스트링(/url?key=value) 형식으로 넘깁니다. (정보가 주소창에 노출됩니다.)
   *       (2) 새로고침을 하더라도 데이터가 유지됩니다. 
   *    2) addFlashAttribute(String key, Object value)
   *       (1) 데이터를 내부적으로 세션(Session)에 임시로 저장하고, 리다이렉트 이후 딱 한 번만 사용할 수 있습니다. (세션에서 자동 소멸)
   *       (2) 정보가 주소창에 노출되지 않습니다. 
   *       (3) 주로 성공이나 실패 메시지, 경고 메시지 등 임시 정보를 전달할 때 사용합니다. 
   */
  
  // RedirectAttributes를 이용한 Redirect
  @RequestMapping("/regist.do") //-------- 요청 주소: /c/regist?title=신규제목&hit=1
  ...
  public String methodE(RedirectAttributes redirectAttrs, int bid, String title, int hit) {
