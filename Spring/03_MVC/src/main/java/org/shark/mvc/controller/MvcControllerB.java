...
public class MvcControllerB {
  ...
  public String methodA(HttpServletRequest request) {
    ...
    // sort에 전달이 없거나, DESC 또는 ASC가 아니면 "DESC" 값을 사용합니다. 
    ...
    int page = 1;
    try {
      page = Integer.parseInt(request.getParameter(sort));
    } catch (Exception e) {
      page = 1;
    }
  ...
  public String methodB(@RequestParam(name="bid") int bid, //------- @RequestParam 어노테이션으로 요청 파라미터를 처리할 수 있습니다. 
                        ...
                        String url) { // @RequestParam 어노테이션이 요청 파라미터를 변수 이름으로 추론하여 결정합니다. 
  ...
  public String methodC(BoardDTO board) { //--- 커맨드 객체를 이용한 요청 파라미터 처리: Constructor 또는 Setter 이용해서 요청 파라미터를 BoardDTO의 멤버변수(필드)에 저장합니다. 
