const questionsHTML = {
  ...
  no8: [
    "태그 이름 규칙(Naming Convention): 태그(Tag)와 속성(Attribute) 이름은 ___ 로 작성합니다.",
  ],
  no9: [
...
const questionsDB = {
  ...
  no14: ["SQL 문의 처리 속도 향상을 위해 특정 칼럼을 대상으로 생성하는 데이터베이스 객체",
    "장점: 검색 속도가 굉장히 빨라질 수 있음",
    "장점: 검색 쿼리의 부하가 줄어들면 곧 시스템 전체 성능이 향상됨",
    "단점: ___를 별도로 저장해야 하기 때문에 DB 크기의 10% 정도 추가 공간이 필요함",
    "단점: 변경 작업(INSERT, UPDATE, DELETE)이 자주 발생하면 오히려 성능이 떨어질 수 있음",
  ],
  no15: ["하나 이상의 기본 테이블이나 다른 _를 이용하여 생성하는 가상 테이블",
    "실제 테이블처럼 행과 열로 구성되지만 실제로 데이터를 가지지는 않음",
    "디스크에 저장한 테이블이 아닌 쿼리만 저장해 둔 형태를 가짐",
    "생성된 _는 SELECT 문을 활용해서 일반 테이블처럼 조회할 수 있음",
  ],
}
...
const questionsJava = {
  no1: ["자바에서 데이터를 저장하는 기본 타입을 의미합니다.",
    "오직 값만 저장하는 타입입니다.",
    "8가지 기본 타입이 있습니다.",
    "객체와 구별하기 위한 데이터 타입입니다.",
    "종류 8가지: byte, short, int, long, float, double, char, boolean",
  ],
  no2: ["값이 아니라 값이 저장된 메모리의 참조값(주소값)을 저장하는 타입입니다.",
    "종류: 클래스 타입, 배열 타입, 인터페이스 타입, 열거 타입 등",
  ],
  no3: ["JVM이 사용하는 메모리 영역 중 하나로 지역 변수, 매개변수 등이 저장되는 공간입니다.",
    "각 스레드마다 독립적으로 존재하고, 메서드가 호출되면 스택 프레임이 쌓이고, 종료되면 제거됩니다.",
    "속도가 빠르고 자동으로 관리되지만, 크기가 제한되어 있어 스택 오버플로우에 주의해야 합니다.",
  ],
  ...
  no6: ["기존 클래스(부모클래스, 슈퍼클래스)의 필드와 메소드를 새로운 클래스(자식클래스, 서브클래스)가 물려받아 재사용할 수 있게 해주는 객체지향 프로그래밍의 핵심 개념입니다.",
    "__을 통해 코드의 재사용성을 높이고, 클래스 간 계층 구조를 명확히 할 수 있습니다.",
    "자바에서는 두 개 이상의 부모를 가지는 다중 __은 불가능합니다.",
  ],
  no7: ["클래스 멤버(필드, 메소드)를 정의할 때 사용하는 키워드입니다.",
    "______으로 선언된 멤버(필드, 메소드)는 객체(인스턴스)에 소속되지 않습니다.",
  ...
  no9: ["상속 관계에 놓인 클래스 중 자식 클래스(서브클래스)가 부모 클래스(슈퍼클래스)를 지칭하는 키워드입니다.",
  ...
  no11: ["Uniform Resource Locator(통합 자원 식별자)를 표현하는 클래스입니다.",
    "파일이나 웹페이지 등 인터넷에 존재하는 자원에 접근할 수 있는 주소(URL)을 구조화하여 다룹니다.",
    "주요 기능으로 URL을 파싱하여 프로토콜, 호스트, 포트, 경로 등 각 구성 요소를 추출할 수 있고", 
    "URLConnection 클래스와 함께 사용하여 실제 네트워크 통신을 수행할 수 있습니다.",
  ],
  no12: ["자바에서 코드에 부가 정보를 표시하기 위한 문법입니다.",
    "실제로 프로그램에 영향을 주는 코드는 아니지만, 컴파일러나 프레임워크 등이 읽고 특별한 동작을 하게 만듭니다.",
    "종류로는 @Override, @Deprecated, @FunctionalInterface 등이 있습니다.",
  ],
  no13: ["객체가 가지는 값입니다.",
    "클래스에서 선언한 변수입니다. 일반 지역변수와 구분해서 __라고 부릅니다.",
    "클래스에 포함된 모든 메소드는 __를 사용할 수 있습니다.",
    "일반 지역 변수와 달리 선언만 하면 자동으로 초기화됩니다. (0, false, null 등)",
  ],
  no14: ["현재 객체 자신을 가리키는 참조 변수입니다.",
    "생성자나 메소드 내부에서 자기 자신 객체를 가리킬 때 사용됩니다.",
    "주로 필드와 지역 변수 이름이 겹칠 때 구분하거나, 메소드 체이닝에서 활용됩니다.",
  ],
  no15: ["객체가 생성될 때 한 번만 호출되는 특별한 메소드입니다.",
    "주로 객체의 초기 상태를 설정(초기화)할 때 사용됩니다.",
    "메소드와 달리 반환 타입이 없고, 클래스 이름과 동일한 이름을 가져야 합니다.",
  ],
  no16: ["클래스 내의 멤버(필드, 메소드)에 접근할 수 있는 권한을 의미합니다.",
    "종류로 pubic, protected, default, private 이 있습니다.",
    "주로 필드: private, 메소드, 생성자: public의 권한을 가집니다.",
  ],
  no17: ["슈퍼클래스(부모클래스)의 메소드를 서브클래스가 다시 만드는 것을 의미합니다.",
    "완전히 동일한 원형을 가지므로 다시 만들어야 합니다. (반환타입, 메소드명, 매개변수가 모두 동일해야 합니다.",
    "_____한 메소드는 @Override 어노테이션을 추가하는 것이 권장됩니다.",
    "접근 제어자의 경우 부모모다 같거나 더 넓은 범위의 권한으로 설정해야 합니다.",
  ],
  no18: ["서브클래스 타입의 객체(인스턴스)를 슈퍼클래스 타입으로 변환할 수 있습니다.",
    "슈퍼클래스 타입으로 변환된 서브클래스 객체는 슈퍼클래스의 멤버에 접근할 수 있습니다.",
    "자동으로 변환되므로 강제로 캐스팅할 필요가 없습니다.",
    "하나의 타입(슈퍼클래스 타입)으로 여러 객체(서브클래스 타입의 객체)를 관리할 수 있습니다. (다형성)",
  ]
}

const answersJava = {
  no1: ["원시값", "Primitive Type",],
  ...
  no11: ["URL", "java.net.URL"],
  no12: ["어노테이션", "Annotation"],
  no13: ["필드", "field"],
  no14: ["this"],
  no15: ["생성자", "constructor"],
  no16: ["접근 제어자", "접근 지시자",],
